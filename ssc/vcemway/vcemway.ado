// Ariel Gu (ariel.gu@northumbria.ac.uk) and Hong Il Yoo (h.i.yoo@durham.ac.uk): 23 June 2019.
// M-Way Clustered Standard Errors
// v1.0.2 (16 Feb 2020)
//	- in Stata 16 or above, some commands (e.g. probit & tobit)do not allow e(N_clust) to be local. when c(version) > 15, we now set the ereturn object to missing to avoid an error message.
// v1.0.1 (22 Jun 2019)
...
program define Estimate, eclass 
	...
	quietly {
		...
		//---------------------------------------
		`command_line' `weight' if `touse' `in', cluster(`cluster1') `options'
		matrix `b'        = e(b)
		matrix `V_oneway' = e(V)
		...
		if (`step4' != 0) {
			...
			forvalues i = 2/`ntuples' {
				`command_line' `weight' if `touse' `in', cluster(`cluster`i'') `options'
				matrix `V`i'' = e(V)
