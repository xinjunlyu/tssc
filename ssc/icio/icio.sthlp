...
{pstd}
	The {cmd:icio_load} command allows to work directly with the most popular ICIO tables - WIOD (Timmer et al. 2015), TIVA (OECD), EORA (Lenzen et al. 2013); in addition, any other user-provided ICIO table can be loaded
	(see the option {opt user()} of {cmd:icio_load}). {cmd:icio} also allows to work with user-defined groups of countries, which means that output measures can be computed  for a country
...
{pstd}
	...
	refines and reconciles the other main contributions in this strand of the literature (Johnson and Noguera, 2012; Wang et al., 2013; Koopman et al. 2014; Borin and Mancini, 2015; Los et al., 2016; Nagengast and Stehrer, 2016; Johnson, 2018; Miroudot and Ye, 2018; Los and Timmer, 2018).
	A key feature of the conceptual scheme proposed by Borin and Mancini (2019) is that different empirical questions call for distinct accounting methods, along with different level of aggregation of trade flows. The icio command shares
	the same rationale.
...
and U the number of uses (i.e., consumption, investment, etc.). As a purely illustrative example, consider the case of importing a IO table from WIOD 2016 release  (indeed, the user does not need to manually import WIOD or TiVA tables,
...
{pstd} Display the value-added originated in a a selected country/sector and absorbed by the final demand of a selected country/sector. All countries or all sectors of origin as well as all countries or all sectors of final demand can be selected at once with the option {opt all}. Examples are:{p_end}
...
constitute the perimeter that matters in deciding whether or not a certain item has to be classified as domestic value-added (GDP) or double-counted. The {opt perspective(exporter)} has to be used also when one seeks a measure of value-added in sectoral and/or bilateral exports that can be added up to the exporter's GDP in its total exports.{p_end}
...
{pstd}
{cmd:icio} is not an official Stata command.  It is a free contribution to the research community.  Please cite it as such:

{phang}
Belotti, F., Borin, A., and Mancini, M. 2019. 
{browse "http://documents.worldbank.org/curated/en/587121582136569876/icio-Economic-Analysis-with-Inter-Country-Input-Output-Tables-in-Stata":icio: Economic Analysis with Inter-Country Input-Output tables in Stata}. Policy Research working paper; No. 9156. World Bank.

{pstd}
When measures of value-added in trade and GVC participation are used, please also cite:

{phang}
Borin, A., and Mancini, M. (2019). {browse "http://documents.worldbank.org/curated/en/639481554384583291/Measuring-What-Matters-in-Global-Value-Chains-and-Value-Added-Trade":Measuring What Matters in Global Value Chains and Value-Added Trade}. Policy Research Working Paper; No. 8804. World Bank.
