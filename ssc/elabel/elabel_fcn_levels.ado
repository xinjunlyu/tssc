*! version 1.4.0 01feb2020 daniel klein
program elabel_fcn_levels , properties(elabel_vvl elabel_aa)
    version 11.2
    
    elabel fcncall define lblnames 0  : `0'
    syntax varlist(max=2) [ if ] [ in ] ///
        [ , Add MODIFY REPLACE APPEND noFIX UNIQue FORCE ]
    ...
    foreach lbl of local lblnamelist {
        elabel copy `tmplbl' `lbl' , `add' `modify' `replace' `append' `fix'
    }
