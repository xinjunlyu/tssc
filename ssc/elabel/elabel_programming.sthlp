...
{pstd}
In the above, {it:lblnamelist} is the expanded {it:elblnamelist}, {varlist} 
is the expanded variable list, and {it:arguments} are {it:arguments} as 
...
{{cmd:define}|{cmd:variable}} {cmd:names 0 : `0'}
{p_end}
{p 14 16 2}
...
{p_end}
{p 14 16 2}
{cmd:elabel} {{cmd:define}|{cmd:variable}} `names' {it:...}
{p_end}
{p 10 12 2}
{cmd:end}
...
{pstd}
{cmd:elabel define} and {cmd:elabel variable} do not parse {it:arguments}, 
{it:iff}, and {it:options}, and they do not define new or modify existing 
value or variable labels; {cmd:elabel_fcn_{it:newfcn}} may, and usually 
does, call {cmd:elabel define} or {cmd:elabel variable}.
...
{p 8 11 2}
2. if none of the above options was specified, {it:lblnamelist} does 
not contain new value label names
