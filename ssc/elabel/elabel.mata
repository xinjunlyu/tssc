*! version 3.6.0 02apr2020 daniel klein
// -------------------------------------- elabel.mata
version 11.2

// -------------------------------------- elabel version
local elabel_version 3.6.0
...
{    
    ...
    // ---------------------------------- private functions
void Elabel_Options__::checkopts()
{
...
// -------------------------------------- Elabel_Options2__
class Elabel_Options2__ extends Elabel_Options__
{
    public :
            void      new()
            void      st_get2()
...
void Elabel_Options2__::new() append = `False'

    // ---------------------------------- public functions
void Elabel_Options2__::st_get2(| `Boolean' st_append)
{
    st_get()
    if ( st_append ) {
        append = (st_local("append") == "append")
        appendok()
    }
    else append = (strpos(st_options()+char(32), " append ")>0)
}

void Elabel_Options2__::appendok()
{
    if ( !append ) return
    if ( modify() | replace() ) {
...
`Boolean' Elabel_Options2__::append() return( append )
...
{    
    ...
    if ( (!rows(attached)) & (k=st_nvar()) ) {
        while ( k ) {
            if ((vlname=st_varvaluelabel(k--)) == "") continue
            if ( !anyof(attached, vlname) ) attached = (attached\ vlname)
        }
        if (mlang) {
            for (k=1; k<=st_nvar(); ++k) {
                if ( st_isstrvar(k) ) continue
                for (l=1; l<=rows(langs()); ++l) {
                    vlname = st_global(sprintf("%s[_lang_l_%s]", 
                                    st_varname(k), langs()[l]) )
                    if ( (vlname != "") & (!anyof(attached, vlname)) )
                        attached = (attached\ vlname)
                }
            }
        }
    }
...
{
    if ( !rows(langs) ) {
        langs = tokens(st_global("_dta[_lang_list]"))'
        if ( !rows(langs) ) langs = clang()
    }
    if ( (langs==clang()) & exclude ) return( J(0, 1, "") )
    return( exclude ? select(langs, (langs:!=clang())) : langs )
}
...
void Elabel_Syntax::get_pvarlist()
{
    ...
    for (i=1; i<=cols(varlist); ++i) {
        ...
        if (lblname != "") {
            ...
            u_exerr(111, "variable %s has no value label attached", 
                                                        varlist[i])
        }
...
void Elabel_ValueLabel::append(| `Boolean' fixfmt)
{
    `RC' vals
    `SC' text
    `RS' i
    ...
    if ( st_vlexists(name()) ) {
        ...
        for (i=1; i<=k(); ++i) {
            ...
            if (st_vlmap(name(), vvec()[i]) == tvec()[i] ) continue
            exit(error(180))
        }
    }
    modify(`False', fixfmt)
...
void Elabel_ValueLabel::_list(| `RS' _name)
{
    ...
    // ---------------------------------- private functions
void Elabel_ValueLabel::keep_last()
{
    `RS'       i
    `BooleanC' keep, seq
    
    if ((i=rows(vvec)) < 2) return
    keep = J(i, 1, `True')
    seq  = order((vvec, (1::i)), (1, 2))
    vvec = vvec[seq]
    while (--i) keep[i] = (vvec[i]!=vvec[i+1]) 
...
class Elabel extends Elabel_Utilities
{
    ...
    private        class Elabel_Options2__ `S' option
    private        class Elabel_ValueLabel `S' vallbl
    ...
    private        `Boolean'               cmd_define_recodingr()
    private        void                    cmd_define_rule()
    ...
    private        void                    cmd_language_saving_c()
    private        void                    cmd_language_saving_l()
...
void Elabel::new() brkintr = querybreakintr()
...
void Elabel::cmd_define()
{
    ...
    if ( cmd_define_recodingr() ) return
    
    if (option.st_options() != "")
        u_st_syntax((option.st_options()+" option"), "[ , OPTION APPEND ]")
...
`Boolean' Elabel::cmd_define_recodingr()
{
    `TS' t
    `RS' popen
    `SS' tok
    ...
    u_st_syntax(option.st_options(), "[ , APPEND noRETURN ]")
    
    if ( cols(syntax.newlblnamelist()) )
        u_exerr(198, "recoding rules not allowed")
    
    cmd_define_rule( (st_local("return")!="noreturn") )
    
    return(`True')
}

void Elabel::cmd_define_rule(`Boolean' returnrules)
{
    `SR'                        lblnamelist
    `BooleanR'                  sel
    ...
    l = Elabel_ValueLabel( cols(lblnamelist) )
    ...
    for (i=1; i<=cols(lblnamelist); ++i) {
        ...
        if ( anyof(text, "") ) text = text :+ st_vlmap(lblnamelist[i], 
            select(r.from(), to)):*(text:==""):*(!select(r.null(), to))
        ...
        l[i].setup(lblnamelist[i], vals, text)
        if ( option.add() & (!option.append()) ) l[i].assert_add()
    }
    
    if ( any(sel) ) cmd_define_oneforeach(select(l, sel))
                             ...
                             `TC' lspec, `Boolean' is_leexp)
{
    ...
    if      ( (!is_veexp) & is_leexp ) { // (numlist) (=eexp)
        ...
        for (i=1; i<=cols(lblnamelist); ++i) {
            ...
            e.eexp(lspec)
            l[i].setup(lblnamelist[i], vspec, e.eexp())
            if ( option.add() & (!option.append()) ) l[i].assert_add()
        }
    }
        ...
        for (i=1; i<=cols(lblnamelist); ++i) {
            ...
            e.eexp(vspec)
            l[i].setup(lblnamelist[i], e.eexp(), lspec)
            if ( l[i].sysmiss() ) exit(error(180))
            if ( option.add() & (!option.append()) ) l[i].assert_add()
        }
    }
        ...
        for (i=1; i<=cols(lblnamelist); ++i) {
            ...
            vals = e.eexp()
            e.eexp(lspec)
            l[i].setup(lblnamelist[i], vals, e.eexp())
            if ( l[i].sysmiss() ) exit(error(180))
            if ( option.add() & (!option.append()) ) l[i].assert_add()
        }
        l = select(l, sel)
...
void Elabel::cmd_define_oneforeach(class Elabel_ValueLabel `R' l)
{
    `RS' nc, i
    ...
    breakoff()
    if ( option.append() )
        for (i=1; i<=nc; ++i) l[i].append(option.fixfmt())
    else if ( option.add()|option.modify() )
        for (i=1; i<=nc; ++i) l[i].modify(option.add(), option.fixfmt())
    else 
        for (i=1; i<=nc; ++i) l[i].define(option.replace(), option.fixfmt())
    breakreset()
...
void Elabel::cmd_list_rvars()
{
    ...
    for (i=1; i<=rows(syntax.langs()); ++i) {
        vars = J(1, 0, "")
        for (k=1; k<=st_nvar(); ++k) {
            if ( st_global(sprintf("%s[_lang_l_%s]", 
                  st_varname(k), syntax.langs()[i]))
            == vallbl.name() ) vars = (vars, st_varname(k))
        }
...
void Elabel::cmd_copy()
{
    ...
    syntax.varvaluelabelok(`True')
    syntax.set(zero)
    u_st_syntax(syntax.options(), "[ , Add MODIFY REPLACE APPEND noFIX ]")
    option.st_get2()
    u_err_notallowed( syntax.mappings() )
    ...
    if ( option.append() ) vallbl.append( option.fixfmt() )
    else if (option.add()|option.modify()) 
        vallbl.modify( option.add(), option.fixfmt() )
...
void Elabel::cmd_save_elabel()
{
    ...
    for (i=1; i<=cols(lblnamelist); ++i) {
        vallbl.setup(lblnamelist[i])
        vallbl.markiff( syntax.iff_eexp() )
        for (j=1; j<=vallbl.k(); ++j) {
            if ( !(rc=_fput(fh, sprintf(`"label define %s %f `"%s"'%s"', 
                                        vallbl.name(), vallbl.vvec()[j],
                                              u_bslsq(vallbl.tvec()[j]),
                      ((j>1)&(option!=", add") ? ", modify" : option))))
               ) continue
            fclose(fh)
            breakreset()
            exit(error(abs(rc)))
...
void Elabel::cmd_language()
{
    ...
    u_st_syntax(zero, "name , SAVing(string asis)")
    lname = st_local("namelist")
    if ( !anyof(syntax.langs(`False'), lname) )
    ...
    if (fileexists(filename)) 
        if (rc=_unlink(filename)) exit(error(abs(rc)))
    if ((fh=_fopen(filename, "w")) < 0) exit(error(abs(fh)))
    fput(fh, sprintf("label language %s, new", lname))
    if (lname == syntax.clang()) cmd_language_saving_c(fh)
    else                  cmd_language_saving_l(lname, fh)
    fclose(fh)
    breakreset()
    printf("{txt}file %s saved\n", filename)
}

void Elabel::cmd_language_saving_c(`RS' fh)
{
    `SS' label, varname
    `SR' done
    `RS' i, j
    ...
    label = u_bslsq(st_local("datalabel"))
    fput(fh, sprintf(`"label data `"%s"'"', label))
    
    pragma unset done
    for (i=1; i<=st_nvar(); ++i) {
        label = u_bslsq(st_varlabel((varname=st_varname(i))))
        fput(fh, sprintf(`"label variable %s `"%s"'"', varname, label))
        if ( st_isstrvar(varname) ) continue
        label = st_varvaluelabel(varname)
        fput(fh, sprintf("label values %s %s", varname, label))
        if ( (label=="")|anyof(done, label) ) continue
        vallbl.setup(label)
        for (j=1; j<=vallbl.k(); ++j) {
            fput(fh, sprintf(`"label define %s %f `"%s"', modify"',
                 vallbl.name(), vallbl.vvec()[j], u_bslsq(vallbl.tvec()[j])))
        }
        done = (done, label)
    }
}

void Elabel::cmd_language_saving_l(`SS' lname, `RS' fh)
{
    `SS' _v_, _l_, label, varname
    `SR' done
    `RS' i, j
       
    _v_ = sprintf("[_lang_v_%s]", lname)
    _l_ = sprintf("[_lang_l_%s]", lname)
    label = st_global("_dta"+_v_)
    fput(fh, sprintf(`"label data `"%s"'"', u_bslsq(label)))
    
    pragma unset done
    for (i=1; i<=st_nvar(); ++i) {
        varname=st_varname(i)
        label = u_bslsq(st_global((varname=st_varname(i))+_v_))
        fput(fh, sprintf(`"label variable %s `"%s"'"', varname, label))
        if ( st_isstrvar(varname) ) continue
        label = st_global(varname+_l_)
        fput(fh, sprintf("label values %s %s", varname, label))
        if ( (label=="")|anyof(done, label) ) continue
        vallbl.setup(label)
        for (j=1; j<=vallbl.k(); ++j) {
                   ...
                   `Boolean' nomemnew)
{
    ...
    if ( !anyof(u.orphans(), oldname) ) {
        ...
        for (i=1; i<=rows(langs); ++i) {
            ...
            (void) l.usedby()
            for (k=1; k<=cols(l.usedby(`False')); ++k) 
                st_varvaluelabel(l.usedby(`False')[k], newname)
        }
                 ...
                 `Boolean' abbrv)
{
    ...
    // ---------------------------------- elabel_vl*()
`TS' elabel_vlinit(`SS' name, | `RC' vvec, `SC' tvec)
{
    class Elabel_ValueLabel `S' l
    if      (args() < 2) l.setup(name)
    else if (args() < 3)  _error(3100)
    else     l.setup(name, vvec, tvec)
    return(l)
}
...
`BooleanR' _aposb(`TR' a, `TR' b, | `RS' d)
{
    ...
    // ---------------------------------- auniq()
`TR' auniq(`TR' a) return( select(a, _auniq(a)) )

    // ---------------------------------- _auniq()
`BooleanR' _auniq(`TR' a)
{
    `BooleanR' v
    `RS'       n, i
    `RC'       o
    ...
    o = order((a', idx), (1, 2))
    for (i=2; i<=n; ++i) v[o[i]] = (a[o[i-1]]!=a[o[i]])
