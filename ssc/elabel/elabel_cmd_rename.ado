*! version 1.2.2 08sep2019 daniel klein
...
void recase_opts(`SS' input, `SS' options)
{
    ...
    elabel_u_st_syntax((", "+options), "[ , Upper Lower Proper ]")
...
void rename_exe(`Rename' r)
{
    ...
    pragma unset clang
    pragma unset langs
    elabel_ldir(clang, langs, `False')
    
    brk = setbreakintr(`False')
    for (i=1; i<=rows(langs); ++i) {
        (void) _stata("_label language "+langs[i], `True')
        for (k=1; k<=st_nvar(); ++k) {
            if ( !anyof(r.oldnames, (vl=st_varvaluelabel(k))) ) continue
            st_varvaluelabel(k, select(r.newnames, (vl:==r.oldnames)))
        }
    }
    if (rows(langs) > 1) (void) _stata("_label language "+clang, `True')
...
void rename_dryrun(`Rename' r)
{
    ...
    col = rowmax((strlen(r.oldnames), 8))+4
