...
version 13.0:
	...
	*/fe xbu dum opar lags(numlist integer) EValopt(varname) ts HGRaph(numlist integer) *]
...
******************************************************************************************************************************************
if "`lags'"!="" | ("`met'"=="xtabond" | "`met'"=="xtdpdsys") loc dyn 1
else loc dyn 0

if "`lags'"=="" loc lyvar 
else {
	loc one 1
	loc lags : list lags-one
	loc lyvar L.`yvar'
	local lyvar=regexr("`lyvar'","L.D.","LD.")
	foreach i of loc lags  {
...
if "`lags'"=="" & ("`met'"=="xtabond" | "`met'"=="xtdpdsys") {
	if "`if'"!="" qui `met' `yvar' `rhsvars' if `timevar'<=`indate' & `if'
	else qui `met' `yvar' `rhsvars' if `timevar'<=`indate'
}
else {
...
forvalues k=1/`ousd' {	
	...
	************************************************************
	...
	if "`lags'"=="" & ("`met'"=="xtabond" | "`met'"=="xtdpdsys") {
		loc lyvar1 L.`yvar' L`yvar'
		loc xvars : list allxvars-lyvar1
	}
	else loc xvars : list allxvars-lyvar
	...
	if `dyn'==0 {
		...
		loc b_lyvar _b[L.`yvar']*L.`y'
		loc b_lyvar_f _b[L.`yvar']*L.`y_f'
		foreach i of loc lags  {
			loc b_lyvar `b_lyvar' +_b[L`i'.`yvar']*L`i'.`y'
			loc b_lyvar_f `b_lyvar_f' +_b[L`i'.`yvar']*L`i'.`y_f'
		}
		
		qui gen `y_f'=`b_lyvar'+`x_c'
		forvalues t = `indate'(1)`cdate' {
			qui replace `y_f'=`x_c'+`b_lyvar_f' if `timevar'==`t'+1 & `y_f'==.
		}
	}
