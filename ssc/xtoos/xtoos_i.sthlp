...
{cmd:xtoos_i} {depvar} [{indepvars}] [{it:if}], 
	[{opt o:us(integer)}] [{opt r:smpl(integer)}] [{opt k:smpl(integer)}]   
	[{opt ev:alopt(varname)}] [{opt m:et}] [{opt mc:omp(string)}] [{opt fe}] [{opt dum}] [{opt lags(numlist)}] 
	[{opt hgraph}] [{it:model_options}]
...
{synopt:{opt lags()}}Specifies the number of lags of the dependent variable that must be included the specification, the default is zero lags.{p_end}
...
Use of {cmd:xtoos_i} including lags of the dependent variable in the specification

{p 4 8 2}{cmd:. xtoos_i n w l.w k l.k ys l.ys, ous(40) rsmpl(20) ksmpl(40) lags(1)}{p_end}
{p 4 8 2}{cmd:. xtoos_i n w l.w k l.k ys l.ys, ous(40) rsmpl(20) ksmpl(40) lags(1/2)}{p_end}
