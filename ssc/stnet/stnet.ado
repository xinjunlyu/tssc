...
program define stnet
	...
	syntax [if] [in] using/ [iw/], Mergeby(namelist) BReaks(string) DIAGdate(varname numeric) BIRTHdate(varname numeric) ///
		    ...
		    LIst(namelist) LISTYEARLY Format(string) noTABles noSHow SAVING(string asis) SAVSTand(string asis) BRENNER INDWEight(varname numeric) Level(integer $S_level) ENDwei ]
...
* Seppa and Pokhrel : expected survival proportion at the mid-point of the interval should provide better weights
				...
				forval ni = 1/`ng' {
					...
					local n = `r(N)' 
					if `n' > 0 {
						su _d if _d == 1 & _t<`end' & `group'==`ni' [iw = `weibr'], meanonly
						local d = `r(sum)' 
						...
						if "`ederer2'"!="" {
							...
							if "`brenner'"!= "" | "`indweight'"!="" {
								su _d if _t<`end' & `group'==`ni' & _d==1 [iw = `weibr'^2], meanonly
								local d_sq = `r(sum)' 							
							}
							else local d_sq = `d'
						}
