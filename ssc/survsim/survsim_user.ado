			...
			if "`loghazard'"!="" | "`hazard'"!="" {
				...
				if "`mixture'"=="" {
					...
					if `nhazvars' {
						...
						global overallsyntax1 real matrix hazvars
					...
					if trim("`first'")!="," {
						...
						if !_rc {
							...
							if `test1'==0 {
								...
								mata: st_local("matachazard",subinstr(st_local("matachazard"),"`first'","hazvars[1,`nhazvars']"))
								if "`ltruncated'"!="" {
									mata: st_local("matachazard0",subinstr(st_local("matachazard0"),"`first'","hazvars[1,`nhazvars']"))
								}
							}
						}
					}
