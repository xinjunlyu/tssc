...
capture program drop summtab
program define summtab
	version 15
	#delimit ;
	syntax [if] [in], [by(varname) total contvars(varlist) catvars(varlist) mean median range pnonmiss rowperc catmisstype(string) pval 
								contptype(integer 1) catptype(integer 1) clustpval clustid(varname) wts(varname) wtfreq(string) fracfmt(integer 2)
								mnfmt(integer 2) medfmt(integer 1) rangefmt(integer 1) pnonmissfmt(integer 1) catfmt(integer 1) pfmt(integer 3)
...
/* |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| */
	...
	/* BY OPTION SPECIFIED ||||||||||||||||||||||||||||||||||| */
	if "`by'" != "" {
	*https://www.stata.com/statalist/archive/2008-08/msg00399.html
...
|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| */
	...
	/* for word table ||||||||||||||||||||||||||||||| */
		if "`word'" == "word" {
			if "`mean'" == "mean" & "`median'" != "median" & "`range'" != "range" & "`pnonmiss'" != "pnonmiss" {
				putdocx table tbl2(`row',.), addrows(2)
			}
			else if "`mean'" == "mean" & "`median'" == "median" & "`range'" != "range" & "`pnonmiss'" != "pnonmiss" {
		...
		if "`excel'" == "excel" {
			...
			local letter : word `i' of `c(ALPHA)'
			local j = `j' + 1
			qui putexcel `letter'`j' = "      N (% Non-missing)"
		}
		/* ||||||||||||||||||||||||||||||||||||||||||||||| */
		...
		/* for word table ||||||||||||||||||||||||||||||| */
		if "`word'" == "word" {
			putdocx table tbl2(`row',1) = ("      N (% Non-missing)"), colspan(2) halign(left)
		}
		local cur_col 2
		local k=1
		...
		if "`mean'" != "mean" & "`median'" != "median" {
			if "`pval'" == "pval" {
				di as error "p-value not allowed when only reporting N and % non-missing"
				exit 198
			}
		}
			...
			qui levelsof `by', local(over_group)
				foreach i of local over_group {
					...
					/* for word table ||||||||||||||||||||||||||||||| */
					if "`word'" == "word" {
						putdocx table tbl2(`row',`cur_col') = ("`pnmiss'"), halign(center)
					}
					local ++cur_col
					...
					if "`excel'" == "excel" {
						local k=`k' + 1
						local letter : word `k' of `c(ALPHA)'
						qui putexcel `letter'`j' = "`pnmiss'", right
					}
					/* ||||||||||||||||||||||||||||||||||||||||||||||| */
				...
				foreach c of local c_l {
					...
					if "`wts'" != "" {
						if "`catmisstype'" == "none" {
							...
							qui tabcount2 `cv' `by' [aweight=`wts'], v1(`rows1') v2(`cols1') matrix(M) zero
						}
						
						else if "`catmisstype'" == "missperc" {
							qui tab `cv' `by' [aweight=`wts'], matcell(M) miss
						}
						...
						if "`catmisstype'" == "none" | "`catmisstype'" == "missperc" {
							local freq_1 = M[`k',`m']
							local freq_1: di %9.`fracfmt'f `freq_1'
						...
						if "`catmisstype'" == "none" {
							qui levelsof `cv', local(rows1)
							qui levelsof `by', local(cols1)
							qui tabcount2 `cv' `by' , v1(`rows1') v2(`cols1') matrix(M) zero
						}
						
						else if "`catmisstype'" == "missperc" {
							qui tab `cv' `by' , matcell(M) miss
						}
			...
			if "`total'" == "total" {
				...
				foreach c of local c_l {
					...
					if "`wts'" != "" {
						if "`catmisstype'" == "none" {
							qui levelsof `cv', local(rows1)
							qui tabcount2 `cv'  [aweight=`wts'], v1(`rows1') matrix(M) zero
						}
						...
						if "`catmisstype'" == "none" {
							qui levelsof `cv', local(rows1)
							qui tabcount2 `cv'  , v1(`rows1') matrix(M) zero
						}
