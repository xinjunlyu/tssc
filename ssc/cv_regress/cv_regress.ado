...
* Step 1. Verify the right command was excecuted in the previous step
	...
	** Estimate estatistcs of interest
	...
	if "`cvwgt'"=="" {
		...
		local mae=r(mean)
		qui:corr `yy' `yloo_hat' if `esample'==1
		matrix c=r(C)
		local pr2=c[1,2]^2
		local rmse=(`mse')^0.5
	}
		...
		local mae=r(mean)
		qui:corr `yy' `yloo_hat' if `esample'==1 [aw=`cvwgt']
		matrix c=r(C)
		local pr2=c[1,2]^2
		local rmse=(`mse')^0.5
	}
	...
	display as text "Root Mean Squared Errors {c |}" _col(30) as result `rmse' %10.3f
	display as text "Mean Absolute Errors     {c |}" _col(30) as result `mae'  %10.5f
