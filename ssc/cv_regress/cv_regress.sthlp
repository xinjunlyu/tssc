...
{synopt:{opth gen(new varname)}} If specified, creates a new variable containing the predicted Leave-one-out error.  y-E(y_-i|X) {p_end}
...
in the estimation of Cross-Validation Statistics.{break} {break}
For the correct implementation, the OLS model needs to be estimated using -regress-, before this program is executed.{break} {break}
cv_regress reports three goodness-of-fit measures: the root mean squared error (RMSE), the mean absolute error (MAE), and the pseudo-R2
(the square of the correlation coefficient of the predicted and observed values of the dependent variable). {break} {break}
It also gives you the option to save the predicted Leave-one-out error, leave one out prediction and the leverage statistic from the model.
 
{title:Saved Results}

cv_regress returns the root mean squared error r(rmse), the mean absolute deviation r(mae), and the pseudo R squared r(pr2). {break}
The program may also create the predicted Leave-one-out error, LOO prediction and leverage statistic for the same sample used in the estimated model.
