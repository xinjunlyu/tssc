...
control units based on the predictors of the outcome. The nonparamentric imputation of the counterfactual is computed using 
...
{phang} {cmd:n_grid}{cmd:(}{it:#}{cmd:)} allows to specify the length of the grid over which finding the optimal bandwidth. 
The default value is 20, which means that the bandwidths' grid is [0.1, 0.2, ..., 2].
...
{pmore}
{inp:r(bandh)} is the bandwidth used within the selected kernel function.

{pmore}
{inp:r(RMSPE)} is the Root Mean Squared Prediction Error of the estimated model.

{pmore}
{inp:r(W)} is the vector of (kernel) weights.


{title:Remarks} 

{pstd} - Before running {cmd:npsynth}, one must first install the {helpb moremata} and {helpb mahapick} modules from SSC. 
...
. npsynth ddva1 $xvars , npscv n_grid(50) panel_var(reporter) ///
  time_var(year) trperiod(2000)  trunit(11) bandw(0.4) kern(triangular) gr1 gr2 gr3  ///
  save_gr1(gr1) save_gr2(gr2) save_gr3(gr3) ///
  gr_y_name("Domestic Direct Value Added Export (DDVA)") gr_tick(5) save_res(NPSYNTH_data) 
...
{psee}
Online: {helpb synth} {helpb mahapick}
{p_end}
