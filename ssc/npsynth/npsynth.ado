...
*! npsynth v13 - GCerulli 16/04/2020
********************************************************************************
cap program drop npsynth
program npsynth, rclass
version 16
#delimit ;     
syntax varlist ,
...
preserve
	...
	*noi sum `Y0'1
	...
	return scalar RMSPE = sqrt(`myRMSPE')
...
return local cmd "npsynth"
return local kern "`kern'"
return local trunit "`trunit'"
return local time_var "`time_var'"
return local panel_var "`panel_var'"
return scalar trperiod=`trperiod'
glo trperiod = `trperiod'
glo trunit = `trunit'
glo kern = "`kern'"
glo bandh = `bandw'
glo rmspe = sqrt(`myRMSPE')
glo vl "`y' `xvars'"
********************************************************************************
...
qui npsynth $vl , panel_var(`r(panelvar)') ///
time_var(`r(timevar)') trperiod($trperiod) ///
trunit($trunit) bandw(`H') kern($kern) 
mat `M'[`i',1]= $bandh
mat `M'[`i',2]= $rmspe
}
qui{
cap drop `M'1 `M'2
svmat `M'
tw (connected `M'2 `M'1) , xtitle("Bandwidths") ytitle("RMSPE") scheme(s2mono)
qui sum `M'2
tempvar min_rmspe
gen `min_rmspe'=r(min)
tempvar dd
